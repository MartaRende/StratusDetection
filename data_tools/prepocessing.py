...
def process_file(fp, datetimes, coordinates, idaweb_data):
    ...
    Extracts weather variables for specified coordinates and merges with IDAWeb data.
    """
    try:
        with Dataset(fp) as nc:
            ...
            for dt_str in datetimes:
                ...
                for name, (ix, iy) in grid_indices.items():
                    ...
                    has_nan = False
                    # Extract weather variables for this point and time
                    for var in ['RR', 'TD', 'WG', 'TT', 'CT', 'FF', 'RS', 'TG', 'Z0', 'ZS', 'DD', 'SU']:
                        try:
                        ...
                        if np.isnan(value):
                            nan_counts[var] += 1
                            has_nan = True
                    # Merge IDAWeb data if available
            ...
            # Print summary of NaN values for each variable
            if nan_counts:
...
def group_data_by_location(processed_data):
    """
    Group processed data entries by location ('nyon', 'dole').
    """
    grouped = {"nyon": [], "dole": []}
...
class PreProcessData:
    """
    Class to preprocess weather and radiation data for a given date range.
    Handles caching, coordinate transformation, and parallel processing.
    """
    def __init__(self, start_date, end_date, fp_inca, fp_global_rayonnement):
    ...
    def _load_or_build_index(self):
        """
        Load datetime-to-file index from cache, or build it if not available.
        """
    ...
    def _build_datetime_index(self):
        """
        Build a mapping from datetime strings to NetCDF file paths.
        """
        self.datetime_index = {}
    ...
    def _preload_idaweb_data(self):
        """
        Load IDAWeb radiation data from text files for 'nyon' and 'dole'.
        """
    ...
    def get_range_data(self, workers=None):
        """
        Process all NetCDF files for the specified date range in parallel.
...
if __name__ == "__main__":
    ...
    processor = PreProcessData(start_date, end_date, fp_inca, fp_global_rayonnement)
    ...
    # Group data by day and save as numpy archive
    ...
    np.savez("data/complete_data.npz", dole=np.array(all_days))
