...
class Metrics:
    ...
    def match_strongest_peaks(
        ...
        # Remove peaks at the first datetime of the day for both expected and predicted peaks
        ...
        for _, exp_row in exp_peaks.iterrows():
            ...
            if not candidates.empty:
                ...
                candidates["combined_score"] = 0.1 * candidates["time_score"] + 0.9 * candidates["slope_score"]
        ...
        print(f"Matched {len(matches)} peaks with {len(exp_peaks)} expected and {len(pred_peaks)} predicted peaks.")
        return result_df